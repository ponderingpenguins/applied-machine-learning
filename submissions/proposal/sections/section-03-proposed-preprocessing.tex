...
We would like to propose a preprocessing pipeline that is organized into two stages. First, we will apply a shared pipeline for all experiments, starting with data quality checks. Since we have confirmed that there are no missing values, we will examine outliers and sensor artifacts (flatline sections or implausible accelerometer/gyroscope spikes). However, rather than automatically removing all detected outliers, we will take a principled approach: we will test the impact of outlier removal on model performance, but we expect to retain most outliers in our feature set. This is motivated by behavioral biometrics literature on gait recognition, which emphasizes that outliers may encode distinctive person-specific patterns. As noted in the gait biometrics survey, it is crucial to recognize that ``on gait data\ldots it underlines how crucial it is to group gait data according to a person's health in order to identify irregularities and classify them'' \cite{kulshreshtha2024survey}. In the context of open-set verification, extreme values in acceleration or rotation may represent individually distinctive motion characteristics that help distinguish the owner from impostors. Segmentation has already been performed on the raw data, and each sample will be represented as a segmented window. The original paper's segmentation strategy uses the aforementioned cumulative acceleration to identify each two-step window, which are of variable length. To create a fixed-length representation, they apply linear interpolation to produce windows of length 128. Many papers, including the original, have found that meaningful segmentations improve predictive accuracy, so we will use this baseline data.
